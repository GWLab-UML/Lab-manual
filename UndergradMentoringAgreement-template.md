# GW Lab Undergraduate Mentoring Agreement 
- *template updated Sep 2025*
- *complete at the beginning of each semester*


Mentee:  

Mentor: 

Semester:

## Working style and commitments

I commit to working ____ hours per week for the ___ semester. My semester class schedule looks like *(insert screenshot)*:


I work best with a (fixed / variable) schedule.
> *(ex. 1-3pm every Monday and Friday / working a total of 5 hours a week at different times)*


When not required to be physically in the lab for lab work, I will primarily work from:


I will organize my lab work, responsibilities, and meeting notes by: 
> *(ex. using Slack canvases, updating electronic lab journal, etc.)*


I best receive feedback by: 


Aside from scheduled meetings, I will check in with my mentor (daily / as needed). I will check in by:


I would like my mentor to check in with me (daily / as needed). 


I prefer to receive Slack messages (between 8am-5pm / anytime).


## Achievements from last summer/semester
What “hard skills” have you learned or practiced: 
> *(ex. streaking agar plates, autoclaving, pipetting, quantifying DNA, species ID, etc.)*


What “soft skills” have you learned or practiced:
> *(ex. time management, prioritizing, asking for help, troubleshooting, team work, etc.)*


## Goals for this summer/semester
“Hard skills” I want to learn or continue to practice: 


“Soft skills” I want to learn or continue to practice:


Bigger picture research goals, and how these have changed or solidified since the last mentoring agreement *(if applicable)*:


Writing goals, and how these have changed or solidified since the last mentoring agreement *(if applicable)*:


I will work towards achieving my goals by: 
> *(taking certain classes, reading papers, informational interviews, etc.)*


Strengths that I will use to achieve my goals: 


Areas I could use improvement in as a scientist and colleague in order to achieve my
goals:


## Future
New experiences I want to gain: 


Subjects I want to learn more about:


Careers/professions I’m interested in:


I would like to learn more about the following opportunities: 


## Mentor’s goals for mentee
> *(to be completed by the mentor after the mentee has filled out all of the above)*




## Agreement has been reviewed:
Mentor Signature:		Date:

Mentee Signature:		Date:
